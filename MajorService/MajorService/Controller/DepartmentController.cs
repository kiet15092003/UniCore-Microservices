...
namespace MajorService.Controller
{
    ...
    public class DepartmentController : ControllerBase
    {
        ...
        public async Task<ApiResponse<bool>> DeleteDepartment(Guid id)
        {
            try
            {
                var result = await _departmentSvc.DeleteDepartmentAsync(id);
            ...
            {
                return ApiResponse<bool>.ErrorResponse(new List<string> { $"An error occurred while deleting the department: {ex.Message}" });
            }
        }
    }
}